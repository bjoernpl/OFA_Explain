...
@dataclass
class AdjustLabelSmoothedCrossEntropyExplCriterionConfig(FairseqDataclass):
...
def label_smoothed_nll_loss(
    ...
    nll_loss = nll_loss.clamp(min=0)

    # Set inf values to 0
    loss = loss.masked_fill(loss == float('inf'), 500)
    nll_loss = nll_loss.masked_fill(nll_loss == float('inf'), 500)
...
@register_criterion(
    "adjust_label_smoothed_cross_entropy_expl", dataclass=AdjustLabelSmoothedCrossEntropyExplCriterionConfig
)
class AdjustLabelSmoothedCrossEntropyCriterionExpl(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, update_num=0, reduce=True):
        ...
        losses = self.compute_loss(model, net_output, sample, update_num, reduce=reduce)
        ans_loss = losses["answer"]["loss"]
        ans_nll_loss = losses["answer"]["nll_loss"]
        ans_ntokens = losses["answer"]["ntokens"]
        expl_loss = losses["explanation"]["loss"]
        expl_nll_loss = losses["explanation"]["nll_loss"]
        expl_ntokens = losses["explanation"]["ntokens"]
        sample_size = (
            sample["target"].size(0) if self.sentence_avg else ans_ntokens
        )
        loss = ans_loss + expl_loss
        nll_loss = ans_nll_loss + expl_nll_loss
        logging_output = {
            "loss": loss.data,
            "nll_loss": nll_loss.data,
            "ans_loss": ans_loss.data,
            "ans_nll_loss": ans_nll_loss.data,
            "expl_loss": expl_loss.data,
            "expl_nll_loss": expl_nll_loss.data,
    ...
    def get_lprobs_and_target(self, model, net_output, sample, constraint_masks=None, conf=1):
        if constraint_masks is not None:
            net_output.masked_fill_(~constraint_masks, -math.inf)
        if self.constraint_start is not None and self.constraint_end is not None:
            net_output[:, :, 4:self.constraint_start] = -math.inf
            net_output[:, :, self.constraint_end:] = -math.inf
    ...
    def compute_loss(self, model, net_output, sample, update_num, reduce=True):
        ...
        # amount of non 1 tokens per sentence in prefix tokens
        prefix_tokens_non_one = prefix_tokens.ne(self.padding_idx).sum(dim=1)
        target = sample["target"]
        answers = sample["answers"]
        answers_non_one = answers.ne(self.padding_idx).sum(dim=1)
        explanations = sample["explanations"]
        explanations_non_one = explanations.ne(self.padding_idx).sum(dim=1)
        losses = {
            "answer": {
                "loss": torch.zeros(sample_size, device=target.device),
                "nll_loss": torch.zeros(sample_size, device=target.device),
                "ntokens": None,
            },
            "explanation": {
                "loss": torch.zeros(sample_size, device=target.device),
        ...
        for i in range(sample_size):
            ...
            losses["answer"]["nll_loss"][i] = ans_nll_loss
            losses["answer"]["ntokens"] = ans_ntokens
            losses["explanation"]["loss"][i] = exp_loss
            losses["explanation"]["nll_loss"][i] = exp_nll_loss
            losses["explanation"]["ntokens"] = exp_ntokens

        if reduce:
            losses["answer"]["loss"] = losses["answer"]["loss"].sum() / sample_size
            losses["answer"]["nll_loss"] = losses["answer"]["nll_loss"].sum() / sample_size
            losses["explanation"]["loss"] = losses["explanation"]["loss"].sum() / sample_size
            losses["explanation"]["nll_loss"] = losses["explanation"]["nll_loss"].sum() / sample_size
        return losses

    def compute_expl_loss(self, model, output, target, sample, update_num, reduce=True, conf=1):
        sample_copy = sample.copy()
        sample_copy["target"] = target
        lprobs, target, constraint_masks = self.get_lprobs_and_target(model, output, sample_copy, conf=conf)
        if constraint_masks is not None:
    ...
    def compute_ans_loss(self, model, output, target, sample, update_num, reduce=True, constraint_masks=None, conf=1):
        sample_copy = sample.copy()
        sample_copy["target"] = target
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        ans_loss_sum = sum(log.get("ans_loss", 0) for log in logging_outputs)
        ans_nll_loss_sum = sum(log.get("ans_nll_loss", 0) for log in logging_outputs)

        expl_loss_sum = sum(log.get("expl_loss", 0) for log in logging_outputs)
        expl_nll_loss_sum = sum(log.get("expl_nll_loss", 0) for log in logging_outputs)
        ...
        metrics.log_scalar(
            "ans_loss", ans_loss_sum / sample_size, ntokens, round=3
        )
        metrics.log_scalar(
            "ans_nll_loss", ans_nll_loss_sum / sample_size, ntokens, round=3
        )
        metrics.log_scalar(
            "expl_loss", expl_loss_sum / sample_size, ntokens, round=3
        )
        metrics.log_scalar(
            "expl_nll_loss", expl_nll_loss_sum / sample_size, ntokens, round=3
        )
        metrics.log_derived(
