...
from data.mm_data.unify_explanation_dataset import UnifyExplanationDataset
...
@dataclass
class UnifyExplanationConfig(OFAConfig):
...
@register_task("unify_explanation", dataclass=UnifyExplanationConfig)
class UnifyExplanationTask(OFATask):
    def __init__(self, cfg: UnifyExplanationConfig, src_dict, tgt_dict):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = UnifyExplanationDataset(
            split,
            dataset,
            self.bpe,
            self.src_dict,
    ...
    def valid_step(self, sample, model, criterion, table=None, **extra_kwargs):
        ...
        explanations = [x[sample['because_idxs'][i]:] for i, x in enumerate(sample["target"])]
        target_expls = [decode(x[x.ne(1)]).strip() for x in explanations]
        val_metrics_split = [
        ...
        for i, (s, ex_s) in enumerate(zip(scores, expl_scores)):
