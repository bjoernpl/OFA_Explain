...
class GeneratorOurs:
    ...
    def handle_co_attn_self(self, block):
        grad = block.get_attention_gradients().detach()
        cam = block.get_attention_map().detach()
        cam = avg_heads(cam, grad)
        R_self_add = apply_self_attention_rules(self.R_self, cam)
        self.R_self += R_self_add

    def handle_co_attn(self, block):
        cam_t_i = block.get_attention_map().detach()
        grad_t_i = block.get_attention_gradients().detach()
        cam_t_i = avg_heads(cam_t_i, grad_t_i)
        R_addition = apply_mm_attention_rules(self.R_t_t, self.R_i_i, self.R_i_t, cam_t_i,
                                                                  apply_normalization=self.normalize_self_attention,
                                                                  apply_self_in_rule_10=self.apply_self_in_rule_10)
        return R_addition


    def generate_ours(self, input, index=None, use_lrp=False, normalize_self_attention=True, apply_self_in_rule_10=True, method_name="ours"):
        ...
        # self.R_t_i = torch.zeros(self.text_tokens, image_patches).to(model.device)
        # # impact of text on images
        # self.R_i_t = torch.zeros(image_patches, self.text_tokens).to(model.device)
...
class GeneratorOursAblationNoAggregation:
    ...
    def handle_co_attn_image(self, block):
        if self.use_lrp:
            ...
            cam_i_t = block.get_attention_map().detach()
        grad_i_t = block.get_attention_gradients().detach()
...
class GeneratorBaselines:
    ...
    def generate_transformer_attr(self, input, index=None, method_name="transformer_attr"):
        ...
        grad_t_i = blk.get_attention_gradients().detach()
        cam_t_i = avg_heads(cam_t_i, grad_t_i)
    ...
    def generate_raw_attn(self, input, method_name="raw_attention"):
        ...
        blk = model.lxmert.encoder.x_layers[-1]
        # cross attn cam will be the one used for the R_t_i matrix
        cam_t_i = blk.get_attention_map().detach()
        cam_t_i = cam_t_i.reshape(-1, cam_t_i.shape[-2], cam_t_i.shape[-1]).mean(dim=0)
        # self.R_t_i = torch.matmul(self.R_t_t.t(), torch.matmul(cam_t_i, self.R_i_i))
        self.R_t_i = cam_t_i
